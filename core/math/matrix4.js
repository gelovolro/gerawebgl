...
export class Matrix4 {
    ...
    static createPerspective(fieldOfViewRadians, aspectRatio, near, far) {
        if (!Number.isFinite(fieldOfViewRadians)
    ...
    static #writePerspectiveIntoUnchecked(out, fieldOfViewRadians, aspectRatio, near, far) {
        ...
        const depthOffset               = Math.fround(
            (MathConstants.MATH_PERSPECTIVE.Z_RANGE_MULTIPLIER * far * near) * inverseDepthRange
        );

        if (!Number.isFinite(horizontalProjectionScale)
            || !Number.isFinite(verticalProjectionScale)
